...
enum MovementType: Int { case RIGHT = 0; case UP = 1; case DIAGONALRIGHTUP = 2; case DIAGONALLEFTUP = 3
    case LEFT = 4; case DOWN = 5; case DIAGONALRIGHTDOWN = 6; case DIAGONALLEFTDOWN = 7; case NONE = 8 }
...
struct Duck {
    ...
    var flyRightAnimation: [SKTexture]?
    var flyRightAction: SKAction?
    var flyRightActionKey: String?

    var flyUpAnimation: [SKTexture]?
    var flyUpAction: SKAction?
    var flyUpActionKey: String?

    var flyDiagonalAnimation: [SKTexture]?
    var flyDiagonalAction: SKAction?
    var flyDiagonalActionKey: String?

    var hurtAnimation: [SKTexture]?
    var hurtAction: SKAction?
    var hurtActionKey: String?

    var deathAnimation: [SKTexture]?
    var deathAction: SKAction?
    var deathActionKey: String?
    ...
    init(duckType: Int,
        ...
        // Animation Set
        setDuckType(extraLife0: extraLife, extraSpeed0: extraSpeed)

        initAnims(initPositionIndex0: initPositionIndex)
    }
    
    mutating func setDuckType(extraLife0: CGFloat, extraSpeed0: CGFloat)
    {
        ...
        case 1:
            ...
            self.life = 2 + Int(extraLife0)
            self.speed = 1 * self.generalSpeed * extraSpeed0
        ...
        case 2:
            ...
            self.life = 1 + Int(extraLife0)
            self.speed = 2 * self.generalSpeed * extraSpeed0
        ...
        case 4:
            ...
            self.life = 3 + Int(extraLife0)
            self.speed = 3 * self.generalSpeed * extraSpeed0
    ...
    mutating func initAnims(initPositionIndex0: Int)
    {
        ...
        self.setInOutAnimation(duckType: self.duckType, initPositionIndex: initPositionIndex0, enter: true)
    }
    ...
    mutating func setInOutAnimation(duckType: Int, initPositionIndex: Int, enter: Bool) {
        ...
        if enter {
            if duckType != 5 { self.firstActionKey = "First" } else {
        ...
        animation0 = assignAnims(_initPositionIndex: initPositionIndex)
        ...
        if enter {
            if initPositionIndex == 0 { self.setScale(scale: CGPoint(x: 1, y: 1)) } else if initPositionIndex == 2 {
                self.setScale(scale: CGPoint(x: -1, y: 1))
            } else if initPositionIndex == 1 {
                self.setScale(scale: CGPoint(x: 1, y: -1))
            }
        } else {
            if initPositionIndex == 0 {
                self.setScale(scale: CGPoint(x: -1, y: 1))
            } else if initPositionIndex == 2 {
                self.setScale(scale: CGPoint(x: 1, y: 1))
            } else if initPositionIndex == 1 {
                self.setScale(scale: CGPoint(x: 1, y: 1))
            }
    ...
    mutating func assignAnims(_initPositionIndex: Int) -> SKAction
    {
        var animation0 = SKAction()
        if duckType == 1 {
            if _initPositionIndex == 0 || _initPositionIndex == 2 {
    ...
    mutating func getMovementType() {
        if direction.x == 1 && direction.y == 0 { self.movementType = MovementType.RIGHT } else if direction.x == -1 && direction.y == 0 { self.movementType = MovementType.LEFT } else if direction.x == 0 && direction.y == 1 { self.movementType = MovementType.UP } else if direction.x == 0 && direction.y == -1 { self.movementType = MovementType.DOWN } else if direction.x == 1 && direction.y == 1 { self.movementType = MovementType.DIAGONALRIGHTUP } else if direction.x == -1 && direction.y == 1 { self.movementType = MovementType.DIAGONALLEFTUP } else if direction.x == 1 && direction.y == -1 { self.movementType = MovementType.DIAGONALRIGHTDOWN } else if direction.x == -1 && direction.y == -1 { self.movementType = MovementType.DIAGONALLEFTDOWN }
    }

    mutating func getDirection() {
        if self.movementType == MovementType.RIGHT { self.direction = CGPoint(x: 1, y: 0) } else if self.movementType == MovementType.LEFT { self.direction = CGPoint(x: -1, y: 0) } else if self.movementType == MovementType.UP { self.direction = CGPoint(x: 0, y: 1) } else if self.movementType == MovementType.DOWN { self.direction = CGPoint(x: 0, y: -1) } else if self.movementType == MovementType.DIAGONALRIGHTUP { self.direction = CGPoint(x: 1, y: 1) } else if self.movementType == MovementType.DIAGONALLEFTUP { self.direction = CGPoint(x: -1, y: 1) } else if self.movementType == MovementType.DIAGONALRIGHTDOWN { self.direction = CGPoint(x: 1, y: -1) } else if self.movementType == MovementType.DIAGONALLEFTDOWN { self.direction = CGPoint(x: -1, y: -1) }
    }
    ...
    mutating func makeAndRunAction() {
        if !isDead() {
            ...
            if self.movementType == .RIGHT || self.movementType == .LEFT {
                ...
                if self.movementType == .LEFT { self.setScale(scale: CGPoint(x: -1, y: 1)) } else { self.setScale(scale: CGPoint(x: 1, y: 1)) }
                ...
                if self.movementType == .DOWN { self.setScale(scale: CGPoint(x: 1, y: -1)) } else { self.setScale(scale: CGPoint(x: 1, y: 1)) }
            } else if self.movementType == .DIAGONALRIGHTUP || self.movementType == .DIAGONALLEFTUP ||
                    self.movementType == .DIAGONALRIGHTDOWN || self.movementType == .DIAGONALLEFTDOWN {
                ...
                if self.movementType == .DIAGONALLEFTUP || self.movementType == .DIAGONALLEFTDOWN { self.setScale(scale:CGPoint(x: -1, y: 1))
                } else if self.movementType == .DIAGONALRIGHTUP || self.movementType == .DIAGONALRIGHTDOWN { self.setScale(scale: CGPoint(x: 1, y: 1)) }
            }
        }
    }
